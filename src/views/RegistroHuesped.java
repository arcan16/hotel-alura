...
public class RegistroHuesped extends JFrame {
	 ...
	 * Create the frame.
	 */
	public RegistroHuesped(Reservacion reservacion, Huesped huesped) {
		this();
		System.out.println(reservacion.getIdReserva());
		System.out.println(huesped.getIdHuesped());
		this.reservacion=reservacion;
		actualizaReservacion(ReservasView.getIdReserva());
		...
		txtNreserva.setText(String.valueOf(huesped.getNumeroReserva()));
		
		btnguardar.removeMouseListener(ml);
		btnguardar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				JOptionPane.showMessageDialog(null, "Fue actualizado el registro con id de reservacion "+reservacion.getIdReserva());
				Huesped huesped2 = new Huesped(huesped.getIdHuesped(), txtNombre.getText(), txtApellido.getText(), txtFechaN.getDate(),
						String.valueOf(txtNacionalidad.getSelectedItem()), txtTelefono.getText(),
						Integer.parseInt(txtNreserva.getText()));

				huespedController = new HuespedController();
				huespedController.actualiza(huesped2);
	...
	public RegistroHuesped() {
		...
		ml= new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!txtTelefono.getText().isEmpty()) {
					System.out.println("Falto el texto");
				}
				if (!txtNombre.getText().isEmpty() && !txtApellido.getText().isEmpty() && txtFechaN.getDate() != null
						&& !txtTelefono.getText().isEmpty()) {
					Huesped huesped = new Huesped(txtNombre.getText(), txtApellido.getText(), txtFechaN.getDate(),
							String.valueOf(txtNacionalidad.getSelectedItem()), txtTelefono.getText(),
							Integer.parseInt(txtNreserva.getText()));
		...
		btnguardar.addMouseListener(ml);
		...
		configurarAccionesFormularioRegistroHuespedes();
	}
	...
	private void configurarAccionesFormularioRegistroHuespedes() {
		if(ReservasView.getIdReserva()!=0) {
			//actualizaReservacion(ReservasView.getIdReserva());
			//JOptionPane.showMessageDialog(null, "Estamos actualizando los registros");			
		}else {
			if (guardaReservacion()) {
				txtNreserva.setText(String.valueOf(ReservacionDao.getIdreservacion()));
			}			
		}
		/*
		 * System.out.println(ReservasView.txtFechaEntrada.getDate());
		 * System.out.println(ReservasView.txtFechaSalida.getDate());
		 * System.out.println(ReservasView.txtValor.getText());
		 * System.out.println(ReservasView.txtFormaPago.getSelectedItem());
		 * System.out.println("Que esta pasando?");
		 */
	}

	private boolean guardaReservacion() {
		// System.out.println(ReservasView.txtValor.getText().replace("$", "").trim());
		Reservacion reservacion = new Reservacion(ReservasView.txtFechaEntrada.getDate(),
		...
		this.reservaController.guardar(reservacion);
		return true;
	}
	
	private void actualizaReservacion(Integer idReserva) {
		this.reservaController = new ReservaController();
		this.reservaController.actualizar(idReserva,reservacion);
	}
